# Strategy

## Unsorted

* Кири: неразвивающие проекты, низкий рейт, много ненужных активностей на инженерах, мотивация
* Макс: UBI fund. мало времени идет на создание внутренних продуктов, мы не можем удовлетворить потребности клиентов существующей экспертизой и ресурсами, как сделать большой системный маркетинг, как зарабатывать 10к на партнера. Проблема не хочу ехать в Сток
* Бодя: 4 инженера и большой админ оверхед на них, нужно больше инженеров
* Алена: системный маркетинг, прогнозируемый поток, демотивация с обработкой новых запросов, confused насчет для чего дизайн спринт - разработка или отдельное направление по консалтингу. Текущая система двоякая – ребята на фултайм не идут.
* Рома: вижу что Алена хочет больше проектов, а инженеры заняты. Может кто-то из инженеров мог бы поуправлять мидлом или джунов, но их нет.
* Стас: проблема выпуска внутреннего продукта. На последней лабе было 3 человека. На предыдущей тоже. Это конечно необязательно, но мб хотим продукт только на бумаге.
* Advisory board 4ire
* Алена: не жопой повернутой к клиентам, клиентоориентированы. Брать проекты. Заработать 30-50к на продукт

  Бодя: подождать

  Рома: не делать ошибок, не жечь фонд. Прикольно когда делали iconx. Классный способ. Мб договориться с застройщиком. Есть много рынков.

  Макс: делать то что создаёт пассивный доход

  Лена: согласна. Прикольная проблема у Боди

* Делать шоу из работы в 4ire
* Качать скиллы продакт менеджера

## Team by [Max](../../org-1/max-semenchuk.md) <a id="&#x1F382;-Financial-Well-Being-by-Max"></a>

TA: Engineers, Designers, Marketers, PMs

### Financial Security

* UBI?

### ⛰️ Self-Fulfillment

* Create a significant impact on society. Solve large, frequent, expensive and common problems
* Have fun & feel ownership over our products
* Get positive feedback, gratitude, recognition \(e.g. speak on conferences\)
* Develop innovative technology, methods, culture, products that have some uniqueness
* Be the professionals \(need deep knowledge for important innovations, grow it further\)
* Be dreamers \(uber was not started by the coolest taxi driver\)
* Be creative and brave \(develop a creative place\)
* Be proud of what we do
* Проработанная идея

{% hint style="info" %}
#### [Lab](https://wiki.4irelabs.com/docs/)

* Solve complex fintech and web3 problems
{% endhint %}

{% hint style="info" %}
#### [Brand](brand.md)
{% endhint %}

### 🎂 Financial Well Being by Max

* More courage and creativity
* Create passive income $10k/mo. per partner
* Enough money in the reserve \(how much?\)
* Lower risks ~regulation
* Product & client diversification
* Бюджет на длительный период

{% hint style="info" %}
#### [Motivation Program](motivation-program.md)
{% endhint %}

{% hint style="info" %}
#### [Internship program](internship.md)
{% endhint %}

{% hint style="info" %}
#### [Partner recruiting campaign](partner-recruiting-campaign.md)
{% endhint %}

{% hint style="info" %}
#### [Lab](https://wiki.4irelabs.com/docs/)

* Develop investable products
{% endhint %}

{% hint style="info" %}
#### [Brand Campaign](brand.md)
{% endhint %}

### [🧸](https://emojipedia.org/teddy-bear/) Comfortable Work Environment by Max

Цели:

* Комфортная нагрузка
* Больше неформальный тусововок, разговоры за кофе
* Хороший скрам \(процесс\)
* Адекватный клиент
* Работать по 8 часов

{% hint style="info" %}
#### [Процесс разработки](https://wiki.4irelabs.com/docs/how-we-work)

* Decrease stress / risks
* Clear start dates etc – меньше проблем с документами/оплатами
{% endhint %}

{% hint style="info" %}
#### [Governance Model](governance.md)
{% endhint %}

{% hint style="info" %}
####  [Brand](brand.md)
{% endhint %}

### ⛲ Professional Development by Max

Цели:

* Interesting projects
* Чувствовать что учишься, развиваешься не только по технологиям, но и бизнес штукам \(Первая компания в которой я чувствутю что учусь. Без развития как без воздуха. Находится в состоянии потока. Задавать себе неудобные вопросы.\)
* Передавать знания
* Построить развивающие процессы внутри
* Привлекать свежую кровь и идеи
* читать книги
* делать эксперименты
* общаться с адвайзорами
* общаться с нетворком, коммьюнити
* брать сложные проекты
* переехать в кремниевую долину \(плотная тусовка\) – Рома хочет поехать

#### Stas

* Writing
* Design thinking

#### Personal Strategies

* Рестроспективные встречи и самоанализ с ребятами
* Личные профессиональные стратегии
* Обсуждать за кальяном / обедом
* Программа по самоорганизации \(холакратия, NVC и тд\). Школа самоорганизации, делать самоорганизовано
* личностные тесты и описать их сильные стороны
* Писать по ребятам в блог про них интересное
* Системы регулярного фидбека
* Выявление запросов
* Дневник осознанности по работе \(рад, грущу, боюсь, инсайт\)
* Развития навыков рефлексии

{% hint style="info" %}
#### [Lab](https://wiki.4irelabs.com/docs/)
{% endhint %}

{% hint style="info" %}
#### [Education fund](education-fund.md)
{% endhint %}

{% hint style="info" %}
#### [Partner recruiting campaign](partner-recruiting-campaign.md)
{% endhint %}

## Fintech Innovators

TA: startup founders, product managers and innovation officers in Nordic countries.

### 💡Create Successful Innovation

Goal: conduct 10 design thinking workshops for our TA in the next 6 month.

Strategy:

1. Education &gt;
2. Community &gt;
3. Taster &gt;
4. Workshop &gt;
5. Development

{% hint style="info" %}
#### Education / Blog

Articles – Max: makes drafts, SBA: rewrites, SEO optimization, Sharing, Analytics, rewrite on Medium \($200-250\), Johan: adds intro in Swedish

* How other companies prioritize and validate ideas? What tools do they use?
* How to proof the idea is worthy?
* How to attract funding?
* Is it enough to have a validates idea \(instead of MVP\) to get funding?
* How to find product/market fit?
* How to make an innovative product?
* How to find market insights?
* Why should I invest in Design Thinking
* Why major players use design sprint?
* How to make a worldwide project in fintech?

Новые идеи:

* Подключить инфлюенсеров \(топ позиции в крупных компаниях\)
* Находить локальных партнеров по B2B \(McKinsey, E&Y\)
* Фокусироваться на наших темам на митапах
* Увеличить масштаб митапов
* Писать о кейсах в Швеции
* Участвовать в Help Reporter

[based on our content plan](https://docs.google.com/spreadsheets/d/1vvkTCBm3xvJ2VYVrQTLo1vZB5MD5BMoU8BofBqpyxiw/edit#gid=1490906225) - discover more about the design sprint + hot topics in FinTech &gt; close to Design sprint\) 

* Webinars
* Interviews
{% endhint %}

{% hint style="info" %}
#### Fintech innovation community  / Meetups

* Meetup: Product Innovation in Fintech \(Sthm, Sep 5\)
* Meetup: Women in Fintech \(Nov\)
* Build Telegram group "Product Innovation in Fintech" \(en\)
* Publish event reports
* Research their needs more
* Lean coffee format
{% endhint %}

{% hint style="info" %}
#### [Design thinking workshop](../../design-thinking/)

Taster by Stas

В финтехе дизайн является решающим факторов

* Calendly \(Max\*, Stas\)
* Ремаркетинг \(FB, Instagram\) – SBA \($450 + $350 ads/mo.\) – Алена
  * Анализ конкурентов и ЦА
  * Настройка и запуск таргетированной рекламы
  * Создание креативов - 4 поста + 4 изображения
  * Еженедельный и ежемесячные отчет о результатах
{% endhint %}

### 🚧 Develop short-term/side IT projects by Stas

* Have capacity for 2-3 projects
* Have 10 RFPs per month

{% hint style="info" %}
#### Project Management
{% endhint %}

{% hint style="info" %}
#### Portfolio
{% endhint %}

{% hint style="info" %}
#### Account management process

Мы вероятнее продадим там где есть партнеры. Канада, Австралия, Нидерланды, Швеция и Сингапур интересный рынок. UK, Германия.

Recycle our base 

* Monthly updates newsletter
* Mark leads based on location, domain, interest + send updates based on interest
{% endhint %}

{% hint style="info" %}
#### Partnership program

Партнеры дают лучших лидов. Партнеры в других странах смогут нам клоузить

* Activities with Chromaway
* Отдельная страница по работе с партнерами
* Строить локальное присутствие Канада, Австралия, Нидерланды, Швеция и Сингапур интересный рынок. UK, Германия.
* Делать роад шоу
* Develop and offer  \(We get referrals / We offer % on the brought revenue\)
* Build a list of potential partners
* Prepare a partners contract / memo
* апдейтить по поездкам и генерить им встречи
* спросить почему нет лидов?
* делать рассылку с нашими апдейтами / Send on monthly basis report/ updates \(Materials, content, company's updates\)
* обмен проектами
* регулярно общаться
* получать наши обновления
* ритвитать / кросс маркетинг
* давать фидбек на наши апдейты
* Найти и подписать партнерские соглашения \(в порядке приоритетов\) на рынках China, HK, Singapore, Nordic, Germany, Austria, Switzerland, UK, US \(NY, SF\)
  1. существующие отношения \(друзья и клиенты\)
  2. Fintech Hubs \(Copenhagen fintech hub\) – Max
  3. Connectors e.g. previous experience in banks \(Stephanie, Sofie..\) – Hele
  4. Agencies consulting, digitalization, fintech \([derivasia.com.sg](http://derivasia.com.sg/) / rubixfintechgroup\) – Helen
  5. Business Consultants \(Avi from Israel...\) – Helen
  6. Fintech startups with whom our partners work with – Helen
  7. Dev companies \(blockchain but not only\) – Helen
  8. Еще через tech carrot.
* Collect feedback to our services in Twitter
{% endhint %}

{% hint style="info" %}
#### Offer
{% endhint %}

### 👯 Save money on the dev team extension by Helen

Hypothesis: Мы сможем растить реализацию через продажу выделенных джунов. С 1 выделенного джуна вероятно сможем нарастить команду.

* message when our experts will be around in your city or conference \(8 &gt; 3 emails\)
* Быть на площадках типа Топтал, Ютим...
* [Flexible rate matrix](https://docs.google.com/spreadsheets/d/1hYPhDO7pR0vf55NTYgXW85MFJ6alB9uu4i7HFd2JLHo/edit#gid=0)
* Расширить количество людей в сейлз
* Мониторить какие проекты поднимают деньги
* Поднимать старых лидов по мобайлу
* Давать тейстер услуг
* Юзать профайлы / линкедины архитектов и писать с их аккаунтов
* Запартнериться с hr компаниями для команд
* Если создать платформу будет много запросов, можно будет отбирать лучшие. Это потребует много денег и ресурсов.

Если холакратия наша фишка, то нужно ставить на это и бренд работодателя

Не хочу заниматься выбиванием комиссионных от партнеров. Поотдавала много проектов. Сложно проверять с клиентом. Мб ресурсы из регионов, на них можно заложить +100%.

Выгодно чтобы когда набираем команду и потом нужно распускать. 

Нужны фулстеки за 3000-3500 в мес.

### 🛴 Be compliant with the regulation by Helen

* Помогать с комплаенсом / Connect with legal partners

### 💵 Get funding by Helen

Есть сделаем инновационное известное решение – это повысить продажи

* Построить образ экспертов / Маркетинг построенный на экспертах. Passive [Consultant brand & profile](https://wiki.4irelabs.com/docs/about/organization) \(linkedin? wiki? other listings?\)
* Вместо оплаты за сервис брать fee от транзакции
* connections with VCs/ accelerators 

#### Develop the Team Capacity

* Поработать с индиго в партнерстве
* Ресурс менеджер ищет проекты заранее когда заканчивается контракт
* Разгрузить Алену
* процессы в лидджен командах
* автоматизация продаж

Risks

* Стремно брать чуваков издалека, особенно при работе с деньгами
* Трудности взаимодействия без знания локального языка/культуры

